...
for(i in 1:(nt/nt_inf)){
  ...
  inf_y1 <- inla.stack(data = list(y = y1[1:(nrow(ES_voronoi_1)*nt_inf),2]),
                       A = list(A_inf_spt_block, 1),
                       effects = list(
                         spde.idx.inf1,
                         list(
                           beta01 = rep(1, nrow(ES_voronoi_1)*nt_inf),
                           u = (1):(nrow(ES_voronoi_1)*nt_inf)   
                         )
                       ),
                       tag = "inf_y1")
  
  # Stack for the second ALR comp.
  inf_y2 <- inla.stack(data = list(y = y2[1:(nrow(ES_voronoi_1)*nt_inf),2]),
                       A = list(A_inf_spt_block, 1),
                       effects = list(
                         spde.idx.inf2,
                         list(
                           beta02 = rep(1, nrow(ES_voronoi_1)*nt_inf),
                           u = (nrow(ES_voronoi_1)*nt_inf+1):(2*nrow(ES_voronoi_1)*nt_inf)
                         )
                       ),
                       tag = "inf_y2")
  ...
  if(i == 1){
    formula_sqc <- y ~ -1 + 
      ...
      f(u, model = "iid2d", n = 2*(nrow(ES_voronoi_1)*nt_inf), constr = TRUE)
    ...
    formula_sqc_mod <- y ~ -1 + 
      ...
      f(u, model = "iid2d", n = 2*(nrow(ES_voronoi_1)*nt_inf), constr = TRUE) # Can not be changed, fixed to a Wishart prior
